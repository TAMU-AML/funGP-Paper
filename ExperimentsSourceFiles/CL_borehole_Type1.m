%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   DESCRIPTION:
%   main function for Cox and Lee (2008) simulation for Type 1 error for borehole function 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
n_eval_points = 10; %number of points at which the functions are evaluated in each dimension
...
logfile = "CL_borehole_Type1.log";
logID = fopen(logfile,'w');

fprintf('Cox and Lee experiment for Type 1 error using borehole function\n')
...
while j <= n_simulation
    ...
    for nrows = 1:length_X
        ...
        Y2_noisy(nrows,:) = borehole([X(nrows,1),X(nrows,2),Tu,Hu,Tl,Hl,L,Kw])+ normrnd(0,10,1,n_replications);
    end

    for replicates = 1:n_replications
        gpMdl1 = fitrgp(X,Y1_noisy(:,replicates),'KernelFunction','ardsquaredexponential');
        Y1_smooth(:,replicates) = predict(gpMdl1,X);
        gpMdl2 = fitrgp(X,Y2_noisy(:,replicates),'KernelFunction','ardsquaredexponential');
